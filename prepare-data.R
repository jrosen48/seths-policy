...
d <- read_rds('downloadedTweets_all_6months.rds') %>% 
  janitor::clean_names() %>% 
  as_tibble()

key = read_csv("summary_data.txt") %>% 
  janitor::clean_names()

state_level_vars <- readxl::read_xlsx("TeachersonTwitter.xlsx") %>% 
  janitor::clean_names()
new_state_level_vars <- read_excel("TwitterTeachersData.xlsx") %>% 
  janitor::clean_names()
...
d_long <- d_long %>% 
  select(user_id, screen_name, created_at, hashtag = hashtags, state_abbre:n_tweets_by_user_in_state)
...
write_csv(d_to_model, "d-to-model.csv")